...
/obj/item/ashtray/throw_impact(atom/hit_atom)
	if (health > 0)
		...
		if (health < 1)
			...
			return
		if (contents.len)
			src.visible_message("<span class='alert'>[src] slams into [hit_atom] spilling its contents!</span>")
...
/obj/item/ashtray/proc/die()
	src.visible_message("<span class='alert'>[src] shatters spilling its contents!</span>")
	for (var/obj/item/clothing/mask/cigarette/O in contents)
		O.loc = src.loc
